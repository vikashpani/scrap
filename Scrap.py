...
CROSSWALK_FILES = [
    r"E:\data\crosswalk_1.xlsx",
    r"E:\data\crosswalk_2.xlsx",
    r"E:\data\crosswalk_3.xlsx",
    r"E:\data\crosswalk_4.xlsx",
]
...
def main():
    ...
    # ============================
    # LOAD & UNION CROSSWALKS
    # ============================
    print("📥 Loading crosswalk files...")
    ...
    for path in CROSSWALK_FILES:
        ...
        if "CLAIM_ID" not in df.columns or "FILENAME" not in df.columns:
            raise ValueError(f"Invalid crosswalk format: {path}")

        crosswalk_dfs.append(df[["CLAIM_ID", "FILENAME"]])

    crosswalk_df = pd.concat(crosswalk_dfs, ignore_index=True).drop_duplicates()

    print(f"✅ Total crosswalk rows: {len(crosswalk_df)}")
    ...
    print(f"📎 Rows with filename: {final_df['FILENAME'].notna().sum()}")
...
# CONFIG
# =========================
CLAIMS_CSV_PATH = "claims.csv"
INPUT_EXCEL_PATH = "input.xlsx"
CHUNK_SIZE = 500_000   # tune based on RAM
...
# CHUNK SEARCH
# =========================
def find_match_in_chunk(chunk: pd.DataFrame, rule_row: pd.Series):
    mask = pd.Series(True, index=chunk.index)

    for input_col, claim_col in COLUMN_MAP.items():
        if input_col not in rule_row or pd.isna(rule_row[input_col]):
            continue

        negate, codes = parse_codes(str(rule_row[input_col]))

        col = chunk[claim_col].astype(str)
...
def search_csv_for_variant(rule_row: pd.Series):
    for chunk in pd.read_csv(
        ...
        match = find_match_in_chunk(chunk, rule_row)
        if match is not None:
            return match  # 🚀 STOP AT FIRST HIT
...
# PROCESS INPUT
# =========================
def process_variants(input_df: pd.DataFrame):
    results = {}
    ...
    for _, rule_row in input_df.iterrows():
        ...
        match = search_csv_for_variant(rule_row)
...
if __name__ == "__main__":
    ...
    results = process_variants(input_df)
...
# ==========================================================
# RANGE EXPANSION (CRITICAL LOGIC)
# ==========================================================
...
- Load superset (first sheet) into SQLite
- Load crosswalk (all sheets) into SQLite
- Generate LLM-assisted column descriptions (Azure Chat)
- Create FAISS index with BGE-small embeddings (persisted)
- Query: node extraction -> semantic retrieval -> map -> LLM SQL compose -> execute -> self-heal
...
# ---------- Config ----------
DB_PATH = "superset.db"
FAISS_DIR = "faiss_index_bge_small"
MAX_SQL_ROWS = 500
...
# ---------- LLM helpers ----------
def llm_generate_column_description(llm, column_name: str, sample_vals: t.List[str], extra_context: str = "") -> tuple[dict, dict]:
    prompt = f"""
You are an expert EDI (837) data analyst.
Given the column name: "{column_name}" and sample values: {sample_vals},
produce JSON ONLY with keys:
- description: a short paragraph describing what values in this column represent (mention likely 837 segments if relevant)
- notes: short notes about format or typical codes
- examples: up to 3 example values

Return JSON only. Context: {extra_context}
"""
    ...
    try:
        ...
        try:
            ...
            parsed = {"description": text, "notes": "", "examples": sample_vals[:3]}
    return parsed, token_usage

def extract_nodes(llm, user_query: str) -> tuple[dict, dict]:
    prompt = f"""
Extract structured nodes from this EDI claims query. Return JSON only:
{{ "nodes": [ {{ "name": "<node_name>", "value":"<value>", "value_type":"exact|approximate|list" }} ], "notes": "" }}
...
return JSON only: {{ "sql": "<SELECT ...>", "params": {{}}, "explanation": ["..."] }}
Limit to {MAX_SQL_ROWS} rows. Use SQLAlchemy named params (:p1).
"""
    resp = llm.generate([[HumanMessage(content=prompt)]])
    text = resp.generations[0][0].text.strip()
    usage = None
...
# ---------- Build docs ----------
def build_crosswalk_docs(crosswalk_excel: str) -> t.List[VectorDoc]:
    sheets = pd.read_excel(crosswalk_excel, sheet_name=None, dtype=object)
    docs = []
    for sheet_name, df in sheets.items():
        clean_name = str(sheet_name).strip().lower().replace(" ", "_")
        df.columns = [str(c).strip().lower().replace(" ", "_") for c in df.columns]
        desc_cols = [c for c in df.columns if "desc" in c or "description" in c or "label" in c or "name" in c]
        for i, row in df.iterrows():
            if desc_cols:
                desc_val = row[desc_cols[0]] if pd.notna(row[desc_cols[0]]) else ""
            else:
                pieces = []
                for c in df.columns[:6]:
                    ...
                    if pd.notna(v):
                        pieces.append(f"{c}:{v}")
                desc_val = " | ".join(pieces)
            text = f"Crosswalk sheet={clean_name} row={i} | {desc_val}"
            meta = sanitize_metadata({"type": "crosswalk", "sheet": clean_name, "row_index": int(i)})
            docs.append(VectorDoc(text=text, metadata=meta))
    return docs

def build_superset_column_docs(superset_excel: str, llm, sample_rows: int = 2, extra_context: str = "") -> tuple[t.List[VectorDoc], t.List[dict]]:
    df = pd.read_excel(superset_excel, sheet_name=0, dtype=object)
    df.columns = [str(c).strip().lower().replace(" ", "_") for c in df.columns]
    docs = []
    usages = []
    for col in df.columns:
        samples = df[col].dropna().astype(str).head(sample_rows).tolist()
        parsed, usage = llm_generate_column_description(llm, col, samples, extra_context=extra_context)
        desc = parsed.get("description") if isinstance(parsed, dict) else str(parsed)
        examples = parsed.get("examples") if isinstance(parsed, dict) else samples[:3]
        text = f"Column: {col}\nDescription: {desc}\nExamples: {examples}"
        meta = sanitize_metadata({"type": "column_description", "column_name": col, "source": Path(superset_excel).name})
        docs.append(VectorDoc(text=text, metadata=meta))
        if usage:
            usages.append(usage)
    return docs, usages

# ---------- FAISS create / load (with dtype safety) ----------
def create_or_load_faiss(all_docs: t.List[VectorDoc], embedding_model, persist_dir: str = FAISS_DIR, force_recreate: bool = False):
    ensure_folder(persist_dir)
    if os.path.exists(persist_dir) and not force_recreate:
        try:
            vectordb = FAISS.load_local(persist_dir, embedding_model)
            return vectordb, {"loaded": True}
        except Exception:
            pass
    texts = [str(d.text) if d.text is not None else "[empty]" for d in all_docs]
    metadatas = [sanitize_metadata(d.metadata) for d in all_docs]
    for i, t in enumerate(texts):
        if t.strip() == "":
            texts[i] = "[empty]"
    # Build vectorstore via LangChain FAISS wrapper (it will call embedding_model.embed_documents)
...
def map_nodes_to_columns(nodes: dict, vectordb, top_k: int = 6) -> dict:
    ...
    for n in nodes.get("nodes", []):
        q = f"node: {n.get('name')} value: {n.get('value')}"
...
def execute_sql_self_heal(engine, sql_json: dict, vectordb, nodes: dict, llm, max_attempts: int = 3):
    attempt = 0
    current_nodes = json.loads(json.dumps(nodes))
    ...
    while attempt <= max_attempts:
        ...
        for s in suggestions:
            ...
            if ":" in txt:
                try:
                    new_val = txt.split(":", 1)[1].strip().split("|")[0].strip()
                except Exception:
                    new_val = None
            if new_val is None:
                m = re.search(r"\b([A-Z0-9\-]{2,12})\b", txt)
                if m:
                    new_val = m.group(1)
            if new_val:
...
# ---------- STREAMLIT UI ----------
st.set_page_config(page_title="EDI Pipeline (BGE-small + Azure LLM)", layout="wide")
st.title("EDI Vector Pipeline — BGE-small embeddings + Azure Chat LLM")

st.sidebar.header("Azure Chat LLM settings (for prompts)")
azure_api_key = st.sidebar.text_input("Azure API Key", type="password", value=os.getenv("AZURE_OPENAI_API_KEY",""))
azure_endpoint = st.sidebar.text_input("Azure Endpoint", value=os.getenv("AZURE_OPENAI_ENDPOINT",""))
azure_api_version = st.sidebar.text_input("Azure API Version", value=os.getenv("AZURE_OPENAI_API_VERSION","2024-02-01"))
chat_deployment = st.sidebar.text_input("Chat deployment name (Azure)", value="")

st.sidebar.markdown("---")
st.sidebar.header("BGE embeddings (HuggingFace)")
bge_model_name = st.sidebar.text_input("HF model (default)", value="BAAI/bge-small-en")
device = st.sidebar.selectbox("Device", ["cpu","cuda"], index=0)
normalize = st.sidebar.checkbox("Normalize embeddings", value=True)

st.sidebar.markdown("---")
st.sidebar.header("Files")
superset_file = st.sidebar.file_uploader("Superset Excel (first sheet used)", type=["xlsx","xls"])
crosswalk_file = st.sidebar.file_uploader("Crosswalk Excel (multi-sheet)", type=["xlsx","xls"])

st.sidebar.markdown("---")
force_recreate = st.sidebar.checkbox("Force recreate FAISS (regenerate descriptions)", value=False)
max_self_heal = st.sidebar.slider("Max self-heal attempts", 0, 3, 2)
k_retrieval = st.sidebar.slider("FAISS retrieval k", 1, 12, 8)

if "ingest_tokens" not in st.session_state:
    st.session_state["ingest_tokens"] = []
if "query_tokens" not in st.session_state:
    st.session_state["query_tokens"] = []

# INGEST
if st.sidebar.button("Ingest & build FAISS"):
    if not superset_file or not crosswalk_file:
        st.sidebar.error("Upload both superset and crosswalk Excel files.")
    elif not (azure_api_key and azure_endpoint and azure_api_version and chat_deployment):
        st.sidebar.error("Provide azure_api_key, endpoint, api_version and chat deployment.")
    else:
        with tempfile.NamedTemporaryFile(delete=False, suffix=".xlsx") as t1:
            t1.write(superset_file.getbuffer()); sup_path = t1.name
        with tempfile.NamedTemporaryFile(delete=False, suffix=".xlsx") as t2:
            t2.write(crosswalk_file.getbuffer()); cw_path = t2.name

        df_sup, sup_table = load_superset_to_sql(sup_path)
        cw_tables = load_crosswalks_to_sql(cw_path)
        st.sidebar.success(f"Loaded superset ({len(df_sup)} rows) and crosswalks: {cw_tables}")

        # Configure Azure env for langchain_openai
        ...
        try:
            ...
            st.sidebar.error(f"Failed to init AzureChatOpenAI: {e}")
            st.stop()

        # create BGE embeddings
        try:
            embed_model = HuggingFaceBgeEmbeddings(model_name=bge_model_name,
                                                   model_kwargs={"device": device},
                                                   encode_kwargs={"normalize_embeddings": normalize})
        except Exception as e:
            st.sidebar.error(f"Failed to init BGE embeddings: {e}")
            st.stop()

        st.sidebar.info("Generating column descriptions via LLM (token usage will be recorded if exposed).")
        sup_docs, usages = build_superset_column_docs(sup_path, llm, sample_rows=2, extra_context="Derived from 837 EDI.")
        st.session_state["ingest_tokens"].extend([u for u in usages if u is not None])
        cw_docs = build_crosswalk_docs(cw_path)
        all_docs = sup_docs + cw_docs

        try:
            vectordb, info = create_or_load_faiss(all_docs, embed_model, persist_dir=FAISS_DIR, force_recreate=force_recreate)
            if info.get("loaded"):
                st.sidebar.success("Loaded existing FAISS index.")
            else:
                st.sidebar.success("Created & persisted FAISS index.")
        except Exception as e:
            st.sidebar.error(f"FAISS creation failed: {e}")
            st.stop()

# QUERY
st.header("Query the superset (natural language)")
user_q = st.text_input("Enter query (e.g. 'institutional claim for orthopaedic cervical ambulatory service')")
...
if st.button("Run Query"):
    if not user_q:
        ...
        if not os.path.exists(FAISS_DIR):
            st.error("No FAISS index found. Ingest first.")
            st.stop()
        if not (azure_api_key and azure_endpoint and azure_api_version and chat_deployment):
            st.error("Azure LLM settings missing.")
            st.stop()
        ...
        embed_model = HuggingFaceBgeEmbeddings(model_name=bge_model_name,
                                              model_kwargs={"device": device},
                                              encode_kwargs={"normalize_embeddings": normalize})
        vectordb = FAISS.load_local(FAISS_DIR, embed_model)

        nodes, usage_nodes = extract_nodes(llm, user_q)
        st.subheader("Extracted nodes")
        st.json(nodes)
        if usage_nodes:
            st.session_state["query_tokens"].append(usage_nodes)

        mapped = map_nodes_to_columns(nodes, vectordb, top_k=k_retrieval)
        st.subheader("Candidate column mappings")
        st.write(mapped)
        ...
        st.write("Final mapping:", final_map)

        sql_json, usage_sql = compose_sql(llm, nodes, final_map)
        st.subheader("Generated SQL")
        st.code(sql_json.get("sql",""))
        st.write("Params:", sql_json.get("params", {}))
        if usage_sql:
            st.session_state["query_tokens"].append(usage_sql)

        engine = create_engine(f"sqlite:///{DB_PATH}")
        rows, details = execute_sql_self_heal(engine, sql_json, vectordb, nodes, llm, max_attempts=max_self_heal)
        st.subheader("Results (limited to 500 rows)")
        if rows:
        ...
        st.subheader("Token usage summary (where available)")
        st.write("Ingest LLM tokens:", sum_tokens(st.session_state["ingest_tokens"]))
        st.write("Query LLM tokens:", sum_tokens(st.session_state["query_tokens"]))

st.caption("BGE-small runs locally on CPU by default. For PHI/production, ensure compliance and secure your keys.")
...
# edi_pipeline_bge_azurellm.py
"""
EDI pipeline: Superset + Crosswalk -> FAISS (HuggingFace BGE embeddings) -> Query (AzureChatOpenAI LLM)
Streamlit app (single-file).

Features:
- Superset Excel -> SQLite
- Crosswalk Excel (all sheets) -> SQLite
- LLM-generated column descriptions (AzureChatOpenAI)
- Embeddings: HuggingFace BGE-large via HuggingFaceBgeEmbeddings
- FAISS vector store (persisted)
- Query: extract nodes -> semantic retrieval -> map -> LLM SQL compose -> execute (LIMIT 500) -> self-heal
- Token usage reporting (where available)
"""
...
# LangChain imports (new package names)
from langchain_openai import AzureChatOpenAI  # chat LLM
from langchain_core.messages import HumanMessage
from langchain_community.vectorstores import FAISS
from langchain_community.embeddings import HuggingFaceBgeEmbeddings
...
# ---------- LLM-driven column description (AzureChatOpenAI) ----------
def llm_generate_column_description(llm, column_name: str, sample_vals: t.List[str], extra_context: str = "") -> tuple[dict, dict]:
    prompt = f"""
You are an expert EDI (837) data analyst.
Given the dataset column name: "{column_name}" and sample values: {sample_vals},
produce JSON ONLY with:
- description: a short paragraph describing what values in this column represent (mention likely 837 segments if relevant)
- notes: short notes about format or typical codes
- examples: up to 3 short example values
...
def create_or_load_faiss(all_docs: t.List[VectorDoc], embedding_model, persist_dir: str = FAISS_DIR, force_recreate: bool = False):
    ...
    for i, t in enumerate(texts):
        if t.strip() == "":
            texts[i] = "[empty]"
    # FAISS.from_texts will call embedding_model.embed_documents internally
    vectordb = FAISS.from_texts(texts=texts, embedding=embedding_model, metadatas=metadatas)
    vectordb.save_local(persist_dir)
    return vectordb, {"loaded": False}

# ---------- Node extraction & SQL composition ----------
def extract_nodes(llm, user_query: str) -> tuple[dict, dict]:
    prompt = f"""
Extract structured search nodes from this user query about EDI claims. Return JSON only:
{{ "nodes": [ {{ "name":"<node_name>", "value":"<value>", "value_type":"exact|approximate|list" }} ], "notes": "" }}
User query: \"{user_query}\"
"""
    resp = llm.generate([[HumanMessage(content=prompt)]])
...
def compose_sql(llm, nodes: dict, mapped_columns: dict, table_name: str = "superset_claims") -> tuple[dict, dict]:
    prompt = f"""
You are a SQL writer. Given table '{table_name}', nodes {json.dumps(nodes)} and mapped_columns {json.dumps(mapped_columns)},
return JSON only: {{ "sql": "<SELECT ...>", "params": {{}}, "explanation": ["..."] }}
Limit to {MAX_SQL_ROWS} rows. Use SQLAlchemy named params (e.g. :p1).
"""
    resp = llm.generate([[HumanMessage(content=prompt)]])
    text = resp.generations[0][0].text.strip()
    usage = None
...
# ---------- STREAMLIT UI ----------
st.set_page_config(page_title="EDI Vector Pipeline - BGE + Azure LLM", layout="wide")
st.title("EDI Vector Pipeline — BGE embeddings + Azure Chat LLM")

st.sidebar.header("Azure Chat LLM settings (api_key, endpoint, api_version required)")
azure_api_key = st.sidebar.text_input("Azure API Key", type="password", value=os.getenv("AZURE_OPENAI_API_KEY",""))
azure_endpoint = st.sidebar.text_input("Azure Endpoint", value=os.getenv("AZURE_OPENAI_ENDPOINT",""))
azure_api_version = st.sidebar.text_input("Azure API Version", value=os.getenv("AZURE_OPENAI_API_VERSION","2024-02-01"))
chat_deployment = st.sidebar.text_input("Chat deployment name (Azure)", value="")
...
st.sidebar.markdown("---")
st.sidebar.header("BGE Embeddings settings (HuggingFace)")
bge_model_name = st.sidebar.text_input("HF model name", value="BAAI/bge-large-en")
device = st.sidebar.selectbox("Device", ["cpu","cuda"], index=0)
normalize = st.sidebar.checkbox("Normalize embeddings", value=True)
...
if st.sidebar.button("Ingest (build FAISS)"):
    if not superset_file or not crosswalk_file:
        ...
        st.sidebar.error("Provide Azure API Key, endpoint, api_version and chat deployment.")
    else:
        # write files temp
        with tempfile.NamedTemporaryFile(delete=False, suffix=".xlsx") as t1:
            t1.write(superset_file.getbuffer()); sup_path = t1.name
        ...
        df_sup, sup_table = load_superset_to_sql(sup_path)
        cw_tables = load_crosswalks_to_sql(cw_path)
        st.sidebar.success(f"Loaded superset ({len(df_sup)} rows) and crosswalk sheets -> {cw_tables}")

        # set Azure envs for langchain_openai if needed
        os.environ["AZURE_OPENAI_API_KEY"] = azure_api_key
        os.environ["AZURE_OPENAI_ENDPOINT"] = azure_endpoint
        os.environ["AZURE_OPENAI_API_VERSION"] = azure_api_version
        ...
        # generate superset column docs (LLM)
        st.sidebar.info("Generating column descriptions (LLM calls) — this will use tokens.")
        sup_docs, llm_usages = build_superset_column_docs(sup_path, llm, sample_rows=2, extra_context="Derived from 837 EDI.")
        st.session_state["ingest_tokens"].extend([u for u in llm_usages if u is not None])

        # build crosswalk docs
        cw_docs = build_crosswalk_docs(cw_path)
...
if st.button("Run Query"):
    if not user_q:
        ...
        # set envs in case changed
        os.environ["AZURE_OPENAI_API_KEY"] = azure_api_key
        os.environ["AZURE_OPENAI_ENDPOINT"] = azure_endpoint
        os.environ["AZURE_OPENAI_API_VERSION"] = azure_api_version

        llm = AzureChatOpenAI(deployment=chat_deployment, temperature=0.0, max_tokens=800)
        embed_model = HuggingFaceBgeEmbeddings(model_name=bge_model_name, model_kwargs={"device": device},
                                              encode_kwargs={"normalize_embeddings": normalize})
        vectordb = FAISS.load_local(FAISS_DIR, embed_model)
...
st.caption("Do not send PHI to non-compliant models. BGE embeddings run locally (device selection).")





# edi_pipeline_rewrite.py
"""
End-to-end EDI superset -> FAISS pipeline (Azure OpenAI embeddings & chat) with Streamlit UI.

Features:
- Load superset (first sheet) to SQLite
- Load crosswalks (all sheets) to SQLite
- Generate LLM-assisted column descriptions (uses first rows + external LLM knowledge)
- Build FAISS index of column descriptions + crosswalk rows (persisted)
- Query pipeline: node extraction -> semantic retrieval -> map -> LLM SQL composition -> execute (LIMIT 500)
- Self-heal loop (configurable attempts)
- Token usage tracking for LLM calls (when available)
- Float32 wrapper for Azure embeddings to solve FAISS unsupported dtype
"""
...
# LangChain / Azure OpenAI imports (packages must be installed)
...
# ----------------- Embeddings wrapper to enforce float32 -----------------
class Float32AzureEmbeddings(AzureOpenAIEmbeddings):
    """
    Subclass AzureOpenAIEmbeddings and coerce outputs to float32 (FAISS requirement).
    """
    def embed_documents(self, texts: t.List[str]) -> t.List[t.List[float]]:
        vectors = super().embed_documents(texts)
        # convert to numpy float32 and back to lists
...
def ensure_vectorstore_folder(folder: str):
...
# ----------------- LLM-based column description -----------------
def llm_generate_column_description(llm, column_name: str, sample_vals: t.List[str], extra_context: str = "") -> tuple[dict, dict]:
    """
    Ask chat LLM to produce a JSON description for a column.
    Returns: parsed_json, token_usage (if provided)
    """
    prompt = f"""
You are an expert EDI/837 data analyst. Given the column name '{column_name}' and sample values {sample_vals},
produce a JSON object with:
- description: one paragraph describing what this column likely contains (mention possible EDI segments like SV1/SV2, HI, CLM, etc. if relevant)
- notes: short notes about format or typical codes
- examples: up to 3 example values

Use your external knowledge about 837 EDI when helpful. Return JSON only.
...
# ----------------- Build docs for crosswalks -----------------
def build_crosswalk_docs(crosswalk_excel: str, llm=None) -> t.List[VectorDoc]:
    sheets = pd.read_excel(crosswalk_excel, sheet_name=None, dtype=object)
    docs = []
    for sheet_name, df in sheets.items():
        clean_name = str(sheet_name).strip().lower().replace(" ", "_")
...
# ----------------- Build superset column docs using LLM -----------------
def build_superset_column_docs(superset_excel: str, llm, sample_rows: int = 2, extra_context: str = "") -> tuple[t.List[VectorDoc], t.List[dict]]:
    df = pd.read_excel(superset_excel, sheet_name=0, dtype=object)
    df.columns = [str(c).strip().lower().replace(" ", "_") for c in df.columns]
    docs = []
...
# ----------------- Persist / Load FAISS safely -----------------
def create_or_load_faiss(all_docs: t.List[VectorDoc], embeddings, persist_dir: str = FAISS_DIR, force_recreate: bool = False):
    ensure_vectorstore_folder(persist_dir)
    # If exist and not forced, try load
    if os.path.exists(persist_dir) and not force_recreate:
        try:
    ...
    for i, t in enumerate(texts):
        if t.strip() == "":
            texts[i] = "[empty]"
    # Build vectorstore (LangChain FAISS will call embeddings.embed_documents)
    vectordb = FAISS.from_texts(texts=texts, embedding=embeddings, metadatas=metadatas)
    vectordb.save_local(persist_dir)
    return vectordb, {"loaded": False}

# ----------------- Node extraction + SQL compose -----------------
def extract_nodes(llm, user_query: str) -> tuple[dict, dict]:
    prompt = f"""
You are an assistant that extracts structured search nodes from a user's free-text request about claims/EDI data.
Return JSON only:
{{ "nodes": [ {{"name":"<node_name>", "value":"<value>", "value_type":"exact|approximate|list"}} ], "notes":"" }}
User query: \"{user_query}\"
"""
...
def compose_sql(llm, nodes: dict, mapped_columns: dict, table_name: str = "superset_claims") -> tuple[dict, dict]:
    prompt = f"""
You are a SQL generator. Given table '{table_name}', nodes: {json.dumps(nodes)} and mapped_columns: {json.dumps(mapped_columns)},
produce JSON only: {{ "sql": "<SELECT ...>", "params": {{}}, "explanation": ["..."] }}
Constraints: limit results to {MAX_SQL_ROWS} rows. Use SQLAlchemy named parameters (:p1) style.
"""
    resp = llm.generate([[HumanMessage(content=prompt)]])
    text = resp.generations[0][0].text.strip()
    usage = None
...
# ----------------- Map nodes -> candidate superset columns (via FAISS) -----------------
def map_nodes_to_columns(nodes: dict, vectordb, top_k: int = 6) -> dict:
    mapped = {}
    for n in nodes.get("nodes", []):
        q = f"node: {n.get('name')} value: {n.get('value')}"
...
def execute_sql_self_heal(engine, sql_json: dict, vectordb, nodes: dict, llm, max_attempts: int = 3):
    ...
    while attempt <= max_attempts:
        ...
        if attempt > max_attempts:
            break
        # Suggest alternatives using crosswalk docs
        suggestions = []
        for n in current_nodes.get("nodes", []):
            q = f"{n.get('name')} {n.get('value')}"
            res = vectordb.similarity_search_with_score(q, k=4)
...
# ----------------- STREAMLIT UI -----------------
st.set_page_config(page_title="EDI Vector Pipeline (Azure+FAISS)", layout="wide")
st.title("EDI Vector Pipeline — Superset + Crosswalk -> FAISS -> Query")

st.sidebar.header("Azure / deployment settings")
st.sidebar.caption("You may set these as environment variables: AZURE_OPENAI_API_KEY, AZURE_OPENAI_ENDPOINT, AZURE_OPENAI_API_VERSION")
azure_endpoint = st.sidebar.text_input("Azure Endpoint (optional)", value=os.getenv("AZURE_OPENAI_ENDPOINT",""))
azure_api_key = st.sidebar.text_input("Azure API Key (optional)", value=os.getenv("AZURE_OPENAI_API_KEY",""), type="password")
azure_api_version = st.sidebar.text_input("Azure API version", value=os.getenv("AZURE_OPENAI_API_VERSION","2024-02-01"))

chat_deployment = st.sidebar.text_input("Chat deployment name (Azure)", value="")
embed_deployment = st.sidebar.text_input("Embedding deployment name (Azure)", value="")

# set envs if provided
if azure_endpoint:
    os.environ["AZURE_OPENAI_ENDPOINT"] = azure_endpoint
if azure_api_key:
    os.environ["AZURE_OPENAI_API_KEY"] = azure_api_key
...
# INGEST action
if st.sidebar.button("Ingest and build FAISS"):
    if not superset_file or not crosswalk_file:
        st.sidebar.error("Upload both superset and crosswalk Excel files.")
        ...
        # 1. SQLite load
        df_sup, table = load_superset_to_sql(sup_path)
        cw_tables = load_crosswalks_to_sql(cw_path)
        st.sidebar.success(f"Loaded superset ({len(df_sup)} rows) and crosswalk tables: {cw_tables}")
        ...
        # 3. build superset column docs (LLM)
        st.sidebar.info("Generating column descriptions (LLM) — this will consume tokens.")
        sup_docs, llm_usages = build_superset_column_docs(sup_path, llm, sample_rows=2, extra_context="Derived from 837 EDI.")
        st.session_state["ingest_tokens"].extend([u for u in llm_usages if u is not None])

        # 4. build crosswalk docs
        cw_docs = build_crosswalk_docs(cw_path)

        # 5. merge docs and create/load FAISS
        all_docs = sup_docs + cw_docs
        try:
            vectordb, info = create_or_load_faiss(all_docs, embeddings, persist_dir=FAISS_DIR, force_recreate=force_recreate)
            if info.get("loaded"):
                st.sidebar.success("Loaded existing FAISS index.")
            else:
                st.sidebar.success("Created & persisted FAISS index.")
...
# QUERY section
st.header("Query the superset via natural language")
user_q = st.text_input("Query (e.g. 'institutional claim for orthopaedic cervical ambulatory service')")
...
if st.button("Run Query"):
    if not user_q:
        ...
        if not chat_deployment or not embed_deployment:
            st.error("Provide chat and embedding deployment names.")
            st.stop()

        # init llm/embeddings and load FAISS
        llm = AzureChatOpenAI(deployment=chat_deployment, temperature=0.0, max_tokens=800)
        embeddings = Float32AzureEmbeddings(deployment=embed_deployment)
        ...
        # 2. map nodes -> candidate columns
        mapped = map_nodes_to_columns(nodes, vectordb, top_k=k_retrieval)
        st.subheader("Candidate column mappings (top results)")
        st.write(mapped)

        # 3. auto map and allow edit
        auto_map = {n: (mapped[n][0]["column"] if mapped.get(n) and len(mapped[n])>0 else None) for n in mapped}
        st.subheader("Auto mapping (editable)")
        final_map = {}
        for node_name, cand in auto_map.items():
        ...
        st.subheader("Token usage (approx, where available)")
        st.write("Ingest LLM token summary:", sum_token_list(st.session_state["ingest_tokens"]))
        st.write("Query LLM token summary:", sum_token_list(st.session_state["query_tokens"]))

st.caption("Note: ensure AZURE_OPENAI_API_KEY, AZURE_OPENAI_ENDPOINT, and deployments are configured. Do not send PHI to non-compliant models.")
...
Single-file prototype using:
- LangChain's AzureChatOpenAI for chat LLM calls
- LangChain's OpenAIEmbeddings (configured for Azure) for embeddings
- FAISS (via langchain.vectorstores.FAISS) for local vector store
- SQLite for superset/crosswalk storage
- Streamlit UI for interaction

Usage:
    streamlit run edi_vector_pipeline_azure_faiss.py
...
# NOTE: Faiss import is a dependency of langchain.vectorstores.FAISS
# Make sure faiss-cpu or faiss-gpu is installed.

# ----- Config / defaults -----
DB_PATH = "superset.db"               # SQLite DB file for superset + crosswalk tables
FAISS_DIR = "faiss_store"             # folder to save/load FAISS index
MAX_SQL_ROWS = 500
...
# ----- Helpers for Azure LangChain initialization -----


def make_llm(deployment_name: str, temperature: float = 0.0, max_tokens: int = 800):
    """
    Create an AzureChatOpenAI model instance using env vars:
      OPENAI_API_KEY, OPENAI_API_BASE, OPENAI_API_VERSION

    Pass deployment_name (the Azure deployment you created).
    """
    llm = AzureChatOpenAI(
...
def make_embeddings(deployment: str, model: str = None):
    """
    Create OpenAIEmbeddings configured for Azure.
    Provide the embedding deployment name.
    """
    # langchain will pick env vars OPENAI_API_TYPE, OPENAI_API_KEY, OPENAI_API_BASE, OPENAI_API_VERSION
    emb = OpenAIEmbeddings(deployment=deployment, model=model)
    return emb


# ----- ETL to SQLite -----
def load_superset_excel_to_sql(excel_path: str, table_name: str = "superset_claims", db_path: str = DB_PATH):
    df = pd.read_excel(excel_path, sheet_name=0, dtype=object)
    df.columns = [str(c).strip().lower().replace(" ", "_") for c in df.columns]
    engine = create_engine(f"sqlite:///{db_path}")
    df.to_sql(table_name, engine, if_exists="replace", index=False)
...
def load_crosswalk_excel_to_sql(excel_path: str, db_path: str = DB_PATH, prefix: str = "crosswalk"):
    sheets = pd.read_excel(excel_path, sheet_name=None, dtype=object)
    engine = create_engine(f"sqlite:///{db_path}")
    created = []
    for sheet_name, df in sheets.items():
        cname = str(sheet_name).strip().lower().replace(" ", "_")
        df.columns = [str(c).strip().lower().replace(" ", "_") for c in df.columns]
        table_name = f"{prefix}_{cname}"
        df.to_sql(table_name, engine, if_exists="replace", index=False)
        created.append(table_name)
    return created


# ----- LLM-driven column description generation -----
def generate_column_description_llm(llm, column_name: str, sample_values: t.List[str]) -> dict:
    prompt = f"""
You are a concise dataset documentation assistant.

Column header: {column_name}
Sample values (up to 5): {sample_values}

Produce JSON only with:
{{
  "description": "<one-line description of the column, what it represents and typical values>",
  "examples": ["ex1", "ex2", ...]  # up to 3 example values
}}
"""
    resp = llm.generate([[HumanMessage(content=prompt)]])
    text = resp.generations[0][0].text.strip()
    try:
        ...
        try:
            ...
            parsed = {"description": f"{column_name} — values like {sample_values[:3]}", "examples": sample_values[:3]}
    return parsed


# ----- Build vector documents from superset and crosswalks -----
def build_docs_from_superset(llm, excel_path: str, sample_rows: int = 2) -> t.List[VectorDoc]:
    df = pd.read_excel(excel_path, sheet_name=0, dtype=object)
    df.columns = [str(c).strip().lower().replace(" ", "_") for c in df.columns]
    docs: t.List[VectorDoc] = []
    for col in df.columns:
        samples = df[col].dropna().astype(str).head(sample_rows).tolist()
        desc = generate_column_description_llm(llm, col, samples)
        text = f"Column: {col}\nDescription: {desc.get('description')}\nExamples: {desc.get('examples')}"
        metadata = {"type": "column_description", "column_name": col, "source": os.path.basename(excel_path)}
        docs.append(VectorDoc(id=f"col_{col}", text=text, metadata=metadata))
    return docs


def build_docs_from_crosswalk(excel_path: str) -> t.List[VectorDoc]:
    sheets = pd.read_excel(excel_path, sheet_name=None, dtype=object)
    docs: t.List[VectorDoc] = []
    for sheet_name, df in sheets.items():
        clean_name = str(sheet_name).strip().lower().replace(" ", "_")
        ...
        for i, row in df.iterrows():
            ...
            text = f"Crosswalk {clean_name} row {i} | " + " | ".join(pieces)
            metadata = {"type": "crosswalk", "sheet": clean_name, "row_index": int(i), "source": os.path.basename(excel_path)}
            docs.append(VectorDoc(id=f"cross_{clean_name}_{i}", text=text, metadata=metadata))
    return docs


# ----- FAISS ingestion (LangChain) -----
def create_faiss_from_docs(docs: t.List[VectorDoc], embeddings, persist_folder: str = FAISS_DIR):
    texts = [d.text for d in docs]
    metadatas = [d.metadata for d in docs]
    ids = [d.id for d in docs]
    # LangChain FAISS.from_texts: returns a FAISS object with index & metadata
    vectordb = FAISS.from_texts(texts=texts, embedding=embeddings, metadatas=metadatas, ids=ids)
    # persist locally
    os.makedirs(persist_folder, exist_ok=True)
    vectordb.save_local(persist_folder)
    return vectordb
...
def load_faiss(persist_folder: str = FAISS_DIR, embeddings=None):
    if not os.path.exists(persist_folder):
        raise FileNotFoundError("FAISS persist folder not found. Ingest first.")
    vectordb = FAISS.load_local(persist_folder, embeddings)
    return vectordb


# ----- Node extraction & SQL composition via LLM (AzureChatOpenAI) -----
def extract_nodes_llm(llm, user_query: str) -> dict:
    prompt = f"""
You are an EDI claims assistant. Convert the user's free-text request into JSON identifying search nodes.
...
{{
  "nodes": [
    {{
      "name": "<node_name>",
      "value": "<value>",
      "value_type": "exact|approximate|list"
    }},
    ...
  ],
...
"""
    ...
    try:
        ...
        try:
            ...
            parsed = {"nodes": [], "notes": "failed_to_parse"}
    return parsed


def compose_sql_llm(llm, nodes: dict, mapped_columns: dict, table_name: str = "superset_claims") -> dict:
    """
    Ask LLM to compose a safe SQL SELECT given nodes and mappings.

    Returns dict: {sql: str, params: dict, explanation: []}
    """
    prompt = f"""
You are a SQL composer. Given:
- table: {table_name}
- nodes: {json.dumps(nodes)}
- mapped_columns (logical -> actual column names): {json.dumps(mapped_columns)}

Produce JSON only:
{{
  "sql": "<parameterized SQL SELECT returning up to {MAX_SQL_ROWS} rows>",
  "params": {{ "p1": "value1", ... }},
  "explanation": ["explain any ambiguous mapping here"]
}}

Constraints:
- Use WHERE for filters.
- Use parameter placeholders suitable for SQLAlchemy (e.g. :p1).
- Return only JSON.
"""
    resp = llm.generate([[HumanMessage(content=prompt)]])
    text = resp.generations[0][0].text.strip()
    try:
        ...
        try:
            ...
            parsed = {"sql": "", "params": {}, "explanation": ["failed_to_parse"]}
    return parsed


# ----- Map nodes to columns using FAISS retrieval (find column_description docs) -----
def map_nodes_to_columns(nodes: dict, vectordb: FAISS, top_k: int = 6) -> dict:
    mapped = {}
    for n in nodes.get("nodes", []):
        node_text = f"node: {n.get('name')} value: {n.get('value')}"
        # similarity search - returns list of Document objects with metadata
        docs_and_scores = vectordb.similarity_search_with_score(node_text, k=top_k)
        candidates = []
        for doc, score in docs_and_scores:
            md = doc.metadata or {}
            if md.get("type") == "column_description":
...
# ----- Execution + self-heal loop -----
def execute_sql_and_self_heal(sql_json: dict, engine, vectordb: FAISS, nodes: dict, llm, max_attempts: int = 3):
    attempt = 0
    ...
    while attempt <= max_attempts:
        attempt += 1
        sql = sql_json.get("sql", "")
        params = sql_json.get("params", {})
        rows = []
        if sql:
            try:
                with engine.connect() as conn:
                    res = conn.execute(text(sql), params)
                    rows = [dict(r) for r in res.fetchmany(MAX_SQL_ROWS)]
            except Exception as e:
                # SQL may be empty/invalid; capture and continue
                print("SQL exec error:", e)
                rows = []
        if rows:
            return rows, {"attempts": attempt, "used_alternatives": used_alternatives}
        # no rows -> self-heal
        if attempt > max_attempts:
            break
        # collect suggestions by querying crosswalk docs
        suggestions = []
        for n in current_nodes.get("nodes", []):
            query_text = f"{n.get('name')} {n.get('value')}"
            results = vectordb.similarity_search_with_score(query_text, k=4)
            for doc, score in results:
                md = doc.metadata or {}
                if md.get("type") == "crosswalk":
                    suggestions.append({"node": n.get("name"), "doc": doc, "score": score})
        if not suggestions:
            break
        # Apply first suggestions heuristically
        for s in suggestions:
            node_name = s["node"]
            doc = s["doc"]
            # find a probable key in doc.text like 'pos: 11' or first numeric token
            doctxt = doc.page_content
            new_val = None
            # quick parse: find pattern "<colname>: <value>"
            if ":" in doctxt:
                parts = doctxt.split("|")[0]  # first segment
                if ":" in parts:
                    try:
                        new_val = parts.split(":", 1)[1].strip().split()[0].strip().strip(",;")
                    except Exception:
                        new_val = None
            # fallback: look for numeric token
            if new_val is None:
                import re
                m = re.search(r"\\b(\\d{2,6}[A-Z0-9-]?)\\b", doctxt)
                if m:
                    new_val = m.group(1)
            if new_val:
                for nn in current_nodes.get("nodes", []):
                    if nn.get("name") == node_name:
                        used_alternatives.append({"node": node_name, "old": nn.get("value"), "new": new_val})
                        nn["value"] = new_val
        # re-map & re-compose SQL
        mapped = map_nodes_to_columns(current_nodes, vectordb)
        mapped_cols = {n: (mapped[n][0]["column"] if mapped.get(n) and len(mapped[n]) > 0 else None) for n in mapped}
        sql_json = compose_sql_llm(llm, current_nodes, mapped_cols)
    return [], {"attempts": attempt, "used_alternatives": used_alternatives}


# ----- Streamlit UI ----- #
st.set_page_config(page_title="EDI Vector + FAISS (Azure)", layout="wide")
st.title("EDI Vector Search + SQL Composer (Azure OpenAI + FAISS)")

st.sidebar.header("Azure/OpenAI settings (env vars must be set)")
llm_deployment = st.sidebar.text_input("Chat LLM deployment name (Azure)", value="")
embed_deployment = st.sidebar.text_input("Embedding deployment name (Azure)", value="")
embedding_model = st.sidebar.text_input("Embedding model name (optional)", value=None)

st.sidebar.markdown("Upload files and then click `Ingest files` to create vectors & persist FAISS.")

superset_file = st.sidebar.file_uploader("Superset Excel (one sheet)", type=["xlsx", "xls"])
crosswalk_file = st.sidebar.file_uploader("Crosswalk Excel (multi-sheet)", type=["xlsx", "xls"])
...
if st.sidebar.button("Ingest files into FAISS"):
    if not superset_file or not crosswalk_file:
        st.sidebar.error("Please upload both superset and crosswalk files.")
    elif not llm_deployment or not embed_deployment:
        st.sidebar.error("Provide both LLM deployment name and embedding deployment name.")
    else:
        # save uploaded files to temp
        with tempfile.NamedTemporaryFile(delete=False, suffix=".xlsx") as t1:
            t1.write(superset_file.getbuffer())
            sup_path = t1.name
        with tempfile.NamedTemporaryFile(delete=False, suffix=".xlsx") as t2:
            t2.write(crosswalk_file.getbuffer())
            cross_path = t2.name
        # 1) load to sqlite
        st.sidebar.info("Loading excels into SQLite...")
        rows_loaded = load_superset_excel_to_sql(sup_path)
        cw_tables = load_crosswalk_excel_to_sql(cross_path)
        st.sidebar.success(f"Loaded superset ({rows_loaded} rows) and crosswalk sheets: {cw_tables}")
        # 2) init llm & embeddings
        st.sidebar.info("Initializing Azure LLM & Embeddings (LangChain)...")
        try:
            llm = make_llm(llm_deployment, temperature=0.0, max_tokens=800)
            embeddings = make_embeddings(deployment=embed_deployment, model=embedding_model)
            ...
            st.stop()
        # 3) build vector docs
        st.sidebar.info("Generating column descriptions (LLM calls)...")
        docs_cols = build_docs_from_superset(llm, sup_path)
        st.sidebar.info("Building crosswalk docs...")
        docs_cross = build_docs_from_crosswalk(cross_path)
        all_docs = docs_cols + docs_cross
        # 4) create FAISS index
        st.sidebar.info("Creating FAISS index (this may take a few seconds)...")
        try:
            vectordb = create_faiss_from_docs(all_docs, embeddings, persist_folder=FAISS_DIR)
            st.session_state["vectordb"] = vectordb
            st.sidebar.success("FAISS index created & persisted.")
        except Exception as e:
            st.sidebar.error(f"FAISS creation failed: {e}")
...
max_self_heal = st.sidebar.slider("Max self-heal attempts", 0, 3, 2)
k_retrieval = st.sidebar.slider("Retrieval k", 1, 12, 8)

st.header("Ask a question about the claims (natural language)")
user_q = st.text_input("Query", value="")
if st.button("Run Query"):
    if not user_q:
        st.error("Enter a query.")
    else:
        if st.session_state.get("vectordb") is None:
            # try to load persisted FAISS if available
            if os.path.exists(FAISS_DIR):
                if not embed_deployment:
                    st.error("Embedding deployment name required to load FAISS. Provide it in the sidebar.")
                    st.stop()
                embeddings = make_embeddings(deployment=embed_deployment, model=embedding_model)
                try:
                    vectordb = load_faiss(FAISS_DIR, embeddings)
                    st.session_state["vectordb"] = vectordb
                    st.session_state["embeddings"] = embeddings
                    st.success("Loaded persisted FAISS index.")
                except Exception as e:
                    st.error(f"Failed to load persisted FAISS: {e}")
                    st.stop()
            else:
                st.error("No vector DB available. Ingest first.")
                st.stop()
        vectordb = st.session_state["vectordb"]
        llm = st.session_state.get("llm")
        if llm is None:
            if not llm_deployment:
                st.error("LLM deployment name required.")
                st.stop()
            llm = make_llm(llm_deployment)
            st.session_state["llm"] = llm
        # 1) node extraction
        with st.spinner("Extracting nodes (LLM)..."):
            nodes = extract_nodes_llm(llm, user_q)
        st.subheader("Extracted nodes")
        st.json(nodes)
        # 2) map nodes to columns
        with st.spinner("Retrieving candidate columns from FAISS..."):
            mapped = map_nodes_to_columns(nodes, vectordb, top_k=k_retrieval)
        st.subheader("Candidate column mappings (top results)")
        st.write(mapped)
        # 3) choose top candidate and allow manual override
        chosen = {n: (mapped[n][0]["column"] if mapped.get(n) and len(mapped[n]) > 0 else None) for n in mapped}
        st.subheader("Auto-chosen mapping (edit if needed)")
        overrides = {}
        for node_name, cand in chosen.items():
            new = st.text_input(f"Column for node '{node_name}'", value=cand or "")
            if new:
                overrides[node_name] = new
        final_mappings = {**chosen, **overrides}
        st.write(final_mappings)
        # 4) SQL compose
        with st.spinner("Composing SQL (LLM)..."):
            sql_json = compose_sql_llm(llm, nodes, final_mappings)
        st.subheader("Generated SQL")
        st.code(sql_json.get("sql", ""))
        st.write("Params:", sql_json.get("params", {}))
        # 5) execute & self-heal
        engine = create_engine(f"sqlite:///{DB_PATH}")
        with st.spinner("Executing SQL and attempting self-heal..."):
            rows, details = execute_sql_and_self_heal(sql_json, engine, vectordb, nodes, llm, max_attempts=max_self_heal)
        st.subheader("Results")
        if rows:
            df = pd.DataFrame(rows)
            st.dataframe(df)
        else:
            st.warning("No results found after attempts.")
        st.subheader("Details & provenance")
        st.write(details)

st.markdown("---")
st.caption("Prototype — test and iterate. Secure keys & PHI before production use.")
